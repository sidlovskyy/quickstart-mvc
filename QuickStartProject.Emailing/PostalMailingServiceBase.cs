...
using Postal;
using Email = Logfox.Domain.Entities.Email;
...
namespace Logfox.Emailing
{
    public class PostalMailingServiceBase
    {
	    ...
	    public void SendEmail(EmailModel email)
		{
			...
			var emailContent = new Email(email.To, email.From, email.CC, data.Subject, data.Body, data.IsBodyHtml, email.Type);
	    ...
	    private static string GetEmailsTemplatePath()
	    {
		    ...
		    string viewsPath = ConfigurationManager.AppSettings["LogFox.EmailTemplatesPath"];
			...
			if(string.IsNullOrWhiteSpace(viewsPath) && HttpContext.Current != null)
			{
				...
				string solutionRootDirectory = GetDirectoryInPath(currentDirectory, "logfox");
				viewsPath = Path.Combine(solutionRootDirectory, @"Logfox.Emailing\Templates");
			}
#endif
